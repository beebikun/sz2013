...
class PlaceVenueExplore(SzApiView):
    ...
    def get(self, request,format=None):
        ...
        place_response = [
            self._serialize_item(place) for place in
            place_service.explore_in_venues(**params)
        ]  
        if place_response:
            data = lebowski_places.PlacesCreate().create(place_response)    
            data['data'] = place_response        
            data['status'] = 200
            return sz_api_response.Response({'data':data['data']},status=data['status'])
        return sz_api_response.Response({'data':place_response})  
